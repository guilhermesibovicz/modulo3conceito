...
app.post("/api/v1/clientes", (req,res)=>{
    ...
    if (!novoCliente.hasOwnProperty('nome')){
        hhtpStatus = 400; //Bad request
    }else{
        // descobrindo o maior ID já cadrastrado de clientes 
        let maiorId = Math.max(...fakeData.map( o => o.id ));
        // validação caso a lista fakeData esteja vazia
        if(maiorId == Infinity) maiorId = 0;
        // adiciono 1 ao maior ID ds clientes e gravo novo Cliente
        novoCliente.id = maiorId + 1;

        // inserir o novo clienet na lista fakeData
        fakeData.push(novoCliente);  //guarda o cliente
    }
    // devolvo o objeto cliente para quem chamou
    res.writeHead(hhtpStatus,{"Content-Type": "application/json"});
